...
class DecoderSatiation:
    ...
    def visualize(self, days, blocks_onsets):
        ...
        plt.suptitle(f'{self.mouse.name} week satiation')
        ...
        plt.savefig(join(self.path, f'{self.mouse.name}_week_satiation.jpg'))
        plt.close()
...
def main():
    for decoder_type in ['svm']:
        path = join(RESULTS_PATH, decoder_type, 'week_satiation')
        if WEEK == 'week1':
