...
PREDICT = False
if PREDICT:
    PATH = join(RESULTS_PATH, 'logistic_regression', 'predict_behavior')
else:
    PATH = join(RESULTS_PATH, 'logistic_regression', 'after_behavior')
...
class LRBehavior:
    ...
    def visualize(self, y_prob, onsets, colors):
    ...
    def run(self):
        ...
        if PREDICT:
            colors = self.analyzer.color_predict_behavior(test_day_data, self.days[self.day_i - 1])
        else:
            colors = self.analyzer.color_after_behavior(test_day_data, self.days[self.day_i - 1])

        self.visualize(y_prob, blocks_onsets, colors)
...
def main():
    for mouse_dict in MICE:
        if DAYS in mouse_dict.keys():
            ...
            for dai_i in [1, 3]:
                process = LRBehavior(mouse, dai_i)
                process.run()
