...
WEEK = 'week2'
DAYS = f'{WEEK}_days'
PATH = join(RESULTS_PATH, 'logistic_regression', 'mixed_days')
...
class LRMixed:
    ...
    def run(self):
        ...
        if WEEK == 'week2':
            blocks_onsets = self.analyzer.blocks_onsets(test_day_data, self.days[self.day_i - 1], self.chunk_size)
        else:
            blocks_onsets = None
