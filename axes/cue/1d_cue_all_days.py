...
N_TRIALS = 30
PATH = join(RESULTS_PATH, 'axes', 'cue', '1d_cue_all_days', WEEK)
...
class AllWeekCueAxes(Processor):
    ...
    def visualize(self, trials_by_axes):
        ...
        titles_trial_type = ['drank', 'ate', 'neutral']
        titles_axes = ['water', 'neutral_water', 'food', 'neutral_food']
        colors_axes = ['darkblue', 'lightgray', 'darkred', 'gray']
        ...
        plt.savefig(join(PATH, f'{self.mouse.name}_1d_cue_axes.jpg'))
        plt.close()

    def select_response(self, cell, index):
        mean_responses = pd.DataFrame([cell[i][:self.sec_hz * 6] for i in index]).mean(axis=0)
        return mean_responses

    def load_day_responses(self, day, vectors):
        test_trials = [day.drank, day.ate, day.neutral]
        vector_types = ['water', 'neutral_water', 'food', 'neutral_food']
        ...
        for trial in test_trials:
            ...
            if len(relevant_trials) > 0:
                with Pool() as pool:
                    ...
                    all_axes_products = [self.analyzer.create_dot_product_iti(pd.DataFrame(trial_responses), vectors[vector_type])
...
def main():
    for mouse_path in RELEVANT_MICE:
        if WEEK in mouse_path.keys():
            ...
            process = AllWeekCueAxes(mouse, vector_name='cue_baseline_axes.npy')
            process.run()
