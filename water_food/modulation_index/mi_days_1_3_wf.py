...
WEEK = f'week1'
DAYS = f'week1_days'
PATH = join(RESULTS_PATH, 'water_food', 'modulation_index', 'days_1_3', 'water_vs_food', ANALYSIS_TYPE)
...
class ModulationIndexWithin:
    ...
    def visualize(self, day1, day3, day2_4):
        all_perc = []
        all_pairs = [day1, day3, day2_4]
        colors = ['darkblue', 'darkred', 'gold']
        labels_names = [['water_1', 'food_1'], ['water_3', 'food_3'], ['water_2', 'food_2']]
        pairs_names = ['day1', 'day3', 'day2_4']

        n_cells = len(day2_4[0][0])
        ...
        plt.suptitle(f'{self.mouse.name}\n day1: {mean_mi[0]}, day3: {mean_mi[1]}, day2_4: {mean_mi[2]}', size=30)

        y_hist = self.visualizer.highest_hist([day1[1], day3[1], day2_4[1]], n_cells, n_bins=N_BINS)
    ...
    def run(self):
        ...
        day2_water = dict_responses['day_2'][f'water_{ANALYSIS_TYPE}']
        day4_food = dict_responses['day_4'][f'food_{ANALYSIS_TYPE}']

        day1 = self.main_analyzer.modulation_index(day1_water, day1_food)
        day3 = self.main_analyzer.modulation_index(day3_water, day3_food)
        day2_4 = self.main_analyzer.modulation_index(day2_water, day4_food)

        all_hist, opposite, mean_values = self.visualize(day1, day3, day2_4)
...
def summary_mice(all_histogram, all_opposite, all_mean):
    ...
    mean_hist = [np.array(pd.DataFrame([m[c] for m in all_histogram if m[c] is not None]).mean(axis=0)) for c in range(3)]
    std_hist = [np.array(pd.DataFrame([m[c] for m in all_histogram if m[c] is not None]
                                      ).std(axis=0) / np.sqrt(len_relevant_hist)) for c in range(3)]

    opposite_values = [[m[c] for m in all_opposite if m[c] is not None] for c in range(3)]
    barplot_opposite = [np.mean(r) for r in opposite_values]
    opposite_pvalue = ttest_ind(opposite_values[0], opposite_values[1])[1]

    mean_values = [[m[c] for m in all_mean if m[c] is not None] for c in range(3)]
    barplot_mean = [np.mean(r) for r in mean_values]
    mean_pvalue = ttest_ind(mean_values[0], mean_values[1])[1]

    colors = ['darkblue', 'darkred', 'gold']
    x_labels = ['day1', 'day3', 'day2_4']

    plt.figure(figsize=(21, 21))
    plt.suptitle(f'Significant cells similarity and correlation', size=30)

    plt.subplot(2, 2, 1)
    plt.title(f'Opposite sign: {round(opposite_pvalue, 2)}', fontsize=25)
    plt.bar(x_labels, barplot_opposite, color=colors, alpha=0.7)
    ...
    plt.subplot(2, 2, 2)
    plt.title(f'Mean: {round(mean_pvalue, 2)}', fontsize=25)
    plt.bar(x_labels, barplot_mean, color=colors, alpha=0.7)
