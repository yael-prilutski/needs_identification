...
class DictMeanResponses:
    ...
    def save_data(self, days_responses):
        ...
        for day in range(len(days_responses)):
            dict_classification[f'day_{day + 1}'] = {
                'water_reward': days_responses[day][0],
                'food_reward': days_responses[day][1]
            }

        np.save(join(self.week_data, 'dict_responses_per_trial.npy'), dict_classification)
    ...
    def select_mean_responses(self, cell, index, onsets):
        ...
        rewards_responses = [bl_sub.iloc[t][onsets[t]: onsets[t] + 2 * self.sec_hz].mean() for t in range(len(bl_sub))]
    ...
    def analyze_rewards(self, day, index_dict):
        ...
        water_rewards, food_rewards = [], []

        with Pool() as pool:
            if len(index_dict['index_drank']) > 3:
                index = index_dict['index_drank']
                onsets_reward = index_dict['onsets_drank']
                water_rewards = pool.starmap(self.select_mean_responses,
                                             [(cell, index, onsets_reward) for cell in cells])

            if len(index_dict['index_ate_full']) > 3:
                index = index_dict['index_ate_full']
                onsets_reward = index_dict['onsets_ate_full']
                food_rewards = pool.starmap(self.select_mean_responses,
                                            [(cell, index, onsets_reward) for cell in cells])

        return water_rewards, food_rewards
...
def main():
    for mouse_dict in MICE:
        if WEEK_TYPE in mouse_dict.keys():
            ...
            processor = DictMeanResponses(mouse)
            processor.run()
